...
static char rcsid[] = "$Id: b4soitemp.c  04/27/2010 Released by Tanvir Morshed $";
    ...
    CKTcircuit *ckt;
{
    ...
    double n0;
	double eggdep, agbc2n, agbc2p, bgbc2n, bgbc2p; /* v4.3.1 bugfix for mtrlMod=1 -Tanvir */
	...
	/*	+ model->B4SOIud * Inv_L*//*Bug #15 Jun 09*/
	   ...
	   /* pParam->B4SOIjtund = pParam->B4SOIidtun * T0; */ 			/*v4.2 redundant */
		...
		T0 = sqrt(2 * Charge_q * epssub);
		SDgamma = T0 * sqrt(fabs(pParam->B4SOInsub))
		/ model->B4SOIcbox;  /* bugfix v4.3.1 -Tanvir */
...
/*              + here->B4SOIagbcpd;    v4.0 */
		...
		{   if (!model->B4SOIvbxGiven){
			T0 = Charge_q / 2 / epssub;
			pParam->B4SOIvbx = pParam->B4SOIphi - T0
		    * pParam->B4SOInpeak
		    * pParam->B4SOIxt * pParam->B4SOIxt;} /* bugfix v4.3.1 -Tanvir */	
		...
		if (!model->B4SOIgamma1Given){
			T0 = sqrt(2 * Charge_q * epssub);	/* bugfix v4.3.1 -Tanvir */
			pParam->B4SOIgamma1 = T0
			* sqrt(pParam->B4SOInpeak)
			/ model->B4SOIcox;}
		if (!model->B4SOIgamma2Given){
			T0 = sqrt(2 * Charge_q * epssub);	/* bugfix v4.3.1 -Tanvir */
		    pParam->B4SOIgamma2 = T0
			* sqrt(pParam->B4SOInsub)
			/ model->B4SOIcox;}
