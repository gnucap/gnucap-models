...
FILE *fplog;
    ...
    {   pParam = here->pParam;
        ...
        fprintf(fplog, "Developers list: https://www.bsim.berkeley.edu/models/bsim4/auth_bsim4/ \n");
        fprintf(fplog, "\n");
    fprintf(fplog, "++++++++++ BSIM4 PARAMETER CHECKING BELOW ++++++++++\n");
    if (fabs(model->BSIM4version - 4.83) > 0.0001){
            fprintf(fplog, "Warning: This model is BSIM4.8.3; you specified a wrong version number.\n");
            printf("Warning: This model is BSIM4.8.3; you specified a wrong version number.\n");
}
    ...
    {   fprintf(fplog, "Fatal: Toxe = %g is not positive.\n",
        ...
        {   fprintf(fplog, "Fatal: TEMPEOT = %g is not positive.\n",
                    model->BSIM4tempeot);
            printf("Fatal: TEMPEOT = %g is not positive.\n", model->BSIM4tempeot);
            Fatal_Flag = 1;
        }
    ...
    {   fprintf(fplog, "Warning: gidlclamp = %g is zero or positive.\n",
            model->BSIM4gidlclamp);
        printf("Warning: gidlclamp = %g is zero or positive.\n", model->BSIM4gidlclamp);
    }
...
/* Check overlap capacitance parameters */
    ...
    {   fprintf(fplog, "Warning: idovvdsc = %g is zero or negative.\n", model->BSIM4idovvdsc);
        printf("Warning: idovvdsc = %g is zero or negative.\n", model->BSIM4idovvdsc);
	}
